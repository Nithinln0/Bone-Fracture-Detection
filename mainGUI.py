...
import cv2  # Added for image validation
...
# Function to validate if the uploaded image is likely an X-ray
def validate_xray(image_path):
    """
    Validate if the uploaded image is likely an X-ray.
    """
    try:
        # Step 1: Check file extension (ensure it's an image format)
        valid_extensions = ['.jpg', '.jpeg', '.png']
        if not any(image_path.lower().endswith(ext) for ext in valid_extensions):
            return False

        # Step 2: Open the image as grayscale
        img = cv2.imread(image_path, cv2.IMREAD_GRAYSCALE)
        if img is None:
            return False

        # Step 3: Check image dimensions (e.g., typical X-ray sizes are large)
        height, width = img.shape
        # Set a minimum dimension for X-ray images (e.g., 512x512 pixels)
        if height < 300 or width < 300:  # Too small to be an X-ray
            return False

        # Step 4: Analyze intensity and variability
        mean_intensity = img.mean()
        std_deviation = img.std()

        # Debugging outputs (optional)
        print(f"File: {image_path}")
        print(f"Mean Intensity: {mean_intensity}, Std Deviation: {std_deviation}")
        print(f"Height: {height}, Width: {width}")

        # Step 5: X-ray images typically have a lower mean intensity and low variability in intensity
        # These ranges are based on typical X-ray image characteristics
        if 20 < mean_intensity < 150 and std_deviation < 50:
            return True
        else:
            return False
...
class App(ctk.CTk):
    def __init__(self):
        ...
        img_resized = img.resize((int(256 / img.height * img.width), 256))  # Resize for display
    ...
    def upload_image(self):
        ...
        if validate_xray(filename):
            ...
            # Display error message if the image is not valid
            messagebox.showerror("Invalid Image", "The selected file is not a valid X-ray image. Please upload a proper X-ray.")
